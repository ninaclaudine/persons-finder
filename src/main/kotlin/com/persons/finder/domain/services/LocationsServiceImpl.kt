...
class LocationsServiceImpl(
    ...
    override fun findAround(latitude: Double, longitude: Double, radiusInKm: Double): Double {

            var miles : Double =  1.15077945
            var lat1: Double = Math.toRadians(latitude)
            var lat2: Double = Math.toRadians(0.0)
            var lon1: Double = Math.toRadians(longitude)
            var lon2: Double = Math.toRadians(0.0)
            ...
            var distancebetween : Double =abs(radiusInKm - distanceinKm)
